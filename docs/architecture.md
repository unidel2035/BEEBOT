...
| stats | выручка, статистика, продажи, отчёт (только ADMIN) |
...
|----------|--------|--------|-----------|
| Тексты (data/texts/) | 20 | ~180 | Основной источник, вручную очищенные |
| YouTube субтитры | 26 | ~60 | Расшифровки видео @a.dmitrov |
...
graph LR
    ...
    subgraph Backend["Backend (FastAPI, 1 384 строки)"]
        AUTH[/api/auth/token]
