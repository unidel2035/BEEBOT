...
class IntegramV2Client:
    ...
    async def _fetch_item_parent_map(self) -> dict[int, int]:
        """REST-список всех позиций → {item_id: order_id (parentId)}.

        REST endpoint возвращает id+parentId без реквизитов (быстро).
    ...
    async def get_order_items(self, order_id: int) -> list[OrderItem]:
        """Получить позиции заказа по order_id через parentId-маппинг."""
        parent_map = await self._fetch_item_parent_map()
        # Берём только item_id принадлежащие этому заказу
    ...
    async def get_order_items_bulk(self) -> list[OrderItem]:
        """Получить ВСЕ позиции заказов (поля из AI tool + parentId из REST)."""
        parent_map = await self._fetch_item_parent_map()
