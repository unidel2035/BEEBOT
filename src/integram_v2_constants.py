"""Константы Integram CRM v2 (ai2o.online) — ID таблиц, реквизитов и lookup-справочники.

Аналог crm_constants.py для новой CRM.
Единый источник истины для всего кода, работающего с ai2o.online.
"""

# ---------------------------------------------------------------------------
# URL и воркспейс
# ---------------------------------------------------------------------------

INTEGRAM_V2_URL = "https://ai2o.online"
INTEGRAM_V2_WORKSPACE = "alekseymavai"

# ---------------------------------------------------------------------------
# ID таблиц
# ---------------------------------------------------------------------------

# Справочники (shared между v1 и v2, ID совпадают)
TABLE_SOURCES = 15          # Источники
TABLE_CATEGORIES = 151      # Категории товаров
TABLE_DELIVERY = 150        # Способы доставки
TABLE_MESSENGERS = 16       # Мессенджеры

# Основные таблицы v2 (созданы 03.04.2026 в ai2o.online)
TABLE_PRODUCTS = 2163       # Товары
TABLE_CLIENTS = 2164        # Клиенты
TABLE_ORDERS = 2165         # Заказы
TABLE_ORDER_ITEMS = 2166    # Позиции заказа (дочерняя к Заказам)

# Chip-таблица статусов (создана 03.04.2026, тип "Статус" = цветные бейджи)
TABLE_STATUS_CHIPS = 21383  # Статус (chip)
TABLE_STATUSES = 21383      # Алиас (для обратной совместимости с импортами)

# История изменений заказа (дочерняя таблица к Заказам)
TABLE_STATUS_HISTORY = 85932  # parentTypeId=2165

# ---------------------------------------------------------------------------
# Реквизиты товаров (таблица 2163)
# ---------------------------------------------------------------------------

REQ_PRODUCT_PRICE = "2180"      # Цена
REQ_PRODUCT_WEIGHT = "2181"     # Вес
REQ_PRODUCT_DESC = "2182"       # Описание
REQ_PRODUCT_INSTOCK = "2183"    # В наличии (BOOL)
REQ_PRODUCT_SKU = "2184"        # Артикул UDS
REQ_PRODUCT_SHORT = "2185"      # Короткое название
REQ_PRODUCT_STOCK = "2186"      # Остаток
REQ_PRODUCT_CATEGORY = "2187"   # Категория (REF → 151)

# ---------------------------------------------------------------------------
# Реквизиты клиентов (таблица 2164)
# ---------------------------------------------------------------------------

REQ_CLIENT_PHONE = "2188"
REQ_CLIENT_TG_ID = "2189"
REQ_CLIENT_TG_USER = "2190"     # Telegram Username
REQ_CLIENT_ADDRESS = "2191"
REQ_CLIENT_CITY = "2192"
REQ_CLIENT_COMMENT = "2193"
REQ_CLIENT_SOURCE = "2194"      # REF → Источники (15)

# ---------------------------------------------------------------------------
# Реквизиты заказов (таблица 2165)
# ---------------------------------------------------------------------------

REQ_ORDER_NUMBER = "2195"            # Номер
REQ_ORDER_DATE = "2196"              # Дата
REQ_ORDER_CLIENT = "2197"            # REF → Клиенты (2164)
REQ_ORDER_SOURCE = "2198"            # REF → Источники (15)
REQ_ORDER_STATUS = "2199"            # REF → Статусы заказов (152)
REQ_ORDER_STATUS_LEGACY = "2199"     # алиас для совместимости (то же поле)
REQ_ORDER_DELIVERY_METHOD = "2200"   # REF → Способы доставки (150)
REQ_ORDER_ADDRESS = "2201"           # Адрес доставки
REQ_ORDER_TRACKING = "2202"          # Трек-номер
REQ_ORDER_ITEMS_TOTAL = "2203"       # Сумма товаров
REQ_ORDER_DELIVERY_COST = "2204"     # Стоимость доставки
REQ_ORDER_TOTAL = "2205"             # Итого
REQ_ORDER_COMMENT = "2206"           # Комментарий

# ---------------------------------------------------------------------------
# Реквизиты позиций заказа (таблица 2166)
# ---------------------------------------------------------------------------

# REQ_ITEM_ORDER — не используется: связь позиции с заказом через parentId (child table)
REQ_ITEM_PRODUCT = "21396"    # REF → Товары (2163) — колонка "Товар" в позициях
REQ_ITEM_QTY = "2209"         # Количество
REQ_ITEM_PRICE = "2210"       # Цена за шт.
REQ_ITEM_SUM = "2211"         # Сумма
REQ_ITEM_UDS_ID = "39181"     # UDS internal product ID (для точной привязки к каталогу)

# ---------------------------------------------------------------------------
# Lookup-справочники (ID записей)
# ---------------------------------------------------------------------------

# Статусы заказов — таблица 152 "Статусы заказов" (REF-колонка 2199 в Заказах)
STATUS_IDS = {
    "Новый": "179",
    "Подтверждён": "181",
    "В сборке": "183",
    "Отправлен": "185",
    "Доставлен": "187",
    "Отменён": "189",
}

# Способы доставки (таблица 150)
DELIVERY_IDS = {
    "СДЭК": "191",
    "Почта России": "193",
    "Самовывоз": "195",
}

# Источники заказов (таблица 15, предсуществующие записи)
SOURCE_IDS = {
    "Telegram": "19",
    "UDS": "21",
    "WhatsApp": "20",
    "ВК": "17",
    "Instagram": "18",
    "Личное обращение": "22",
    "Сообщество": "23",
}

# Категории товаров (таблица 151)
CATEGORY_IDS = {
    "Продукты пчеловодства": "157",
    "Настойки": "159",
    "Программы здоровья": "161",
    "Мёд": "163",
    "Наборы": "165",
    "Упаковка": "167",
    "Свечи": "169",
    "Чаи и травы": "171",
}

# Списки для справочников
ORDER_STATUSES = list(STATUS_IDS.keys())
DELIVERY_METHODS = list(DELIVERY_IDS.keys())
